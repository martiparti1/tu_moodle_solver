...
export default function UserList({users, setUsers}){
    ...
    async function handleToggleSpecialClick(e){
        let toggle_this = e.currentTarget.closest('div[data-id]')?.dataset.id;

        await axios.post('http://localhost:3000/toggle-special', {user_id : toggle_this});

        setUsers(prev => prev.map(user => 
            user.id = toggle_this ? {...user, isSpecial : !isSpecial} : user
        ))
    }
            ...
            {users.map((user) => 
                <div key = {user.id} data-id = {user.id} className="flex flex-col min-w-[30dvw] bg-gray-300 rounded-lg my-[10px] p-[5px] shadow-lg">
                    ...
                    <div className="flex flex-row justify-between">
                        ...
                        <button
                            className="bg-cyan-400 text-white rounded-lg px-[7px] active:scale-95 min-h-[30px]"
                            onClick={handleToggleSpecialClick}
                        >
                            toggle special
                        </button>
                    </div>
